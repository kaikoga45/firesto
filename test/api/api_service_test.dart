...
void main() {
  ...
  const String dummyJsonSearchRestaurant =
  ...
  const String dummyJsonDetailRestaurant =
  ...
  const String dummyJsonUserRestaurantReview =
      '{"error":false,"message":"success","customerReviews":[{"name":"Ahmad","review":"Tidak rekomendasi untuk pelajar!","date":"13 November 2019"},{"name":"Kai","review":"Test","date":"6 September 2021"}]}';

  group(
    'Testing Api Service Parsing JSON into Model',
    () {
      test(
        ...
        () async {
          when(
            ...
            (_) async => http.Response(dummyJsonListRestaurant, 200),
          );

          final ListRestaurantResult result =
              await apiService.getListRestaurant();

          expect(
            result,
            isA<ListRestaurantResult>(),
          );
        },
      );
      ...
      test(
        ...
        () async {
          const String restaurantName = 'Makan mudah';

          when(
            apiService.client.get(
              Uri.parse(baseUrl + search + restaurantName),
            ),
          ).thenAnswer(
            (_) async => http.Response(dummyJsonSearchRestaurant, 200),
          );

          final SearchRestaurantResult result =
              await apiService.getSearchRestaurant(restaurantName);
      ...
      test(
        'Get detail of restaurant based on id',
        () async {
          ...
          when(
            ...
            (_) async => http.Response(dummyJsonDetailRestaurant, 200),
          );

          final DetailRestaurantResult result =
      ...
      test(
        'Post user restaurant review',
        () async {
          const String restaurantId = 'rqdv5juczeskfw1e867';
          const String userName = 'Kai';
          const String userReview = 'Not bad for reasonably price';
          ...
          when(
            ...
            (_) async => http.Response(dummyJsonUserRestaurantReview, 200),
          );

          final CustomerReviewResult result =
              await apiService.postRestaurantReview(
            id: restaurantId,
            review: userReview,
          );
